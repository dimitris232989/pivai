...
(function () {
  ...
  (function () {
    ...
    if (langMenu) {
      qsa('button[data-lang]', langMenu).forEach(function (b) {
        on(b, 'click', function () {
          var newCode = b.getAttribute('data-lang') || 'EN';
          var codeEl = langWrap ? qs('.code', langWrap) : null;
          var prevCode = codeEl ? codeEl.textContent.trim() : 'EN';

          // Swap: put the previous active language into the clicked item's slot
          b.setAttribute('data-lang', prevCode);
          b.textContent = prevCode;

          // Update the toggle button to show the newly selected language
    ...
    qsa('.mega3').forEach(function (panel) {
      ...
      qsa('.mega-link', panel).forEach(function (link) {
        ...
        // Store dataset selection in sessionStorage before navigating to case-studies page
        on(link, 'click', function () {
          var ds = link.getAttribute('data-cs-dataset');
          if (ds) {
            sessionStorage.setItem('pivai_dataset', ds);
          } else {
            sessionStorage.removeItem('pivai_dataset');
          }
        });
      });
    });
  ...
  (function () {
    ...
    var photoEl = qs('.team-bio__photo', panel);
    ...
    function openPanelFor(btn) {
      ...
      if (photoEl) {
        var img = btn.querySelector('.member__photo');
        photoEl.src = img ? img.src : '';
        photoEl.alt = btn.getAttribute('data-name') || '';
      }
